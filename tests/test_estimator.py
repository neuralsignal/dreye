...
def test_introduction():
    ...
    fig, axes = est.gamut_plot(
        np.atleast_2d(Q), colors=sources_colors, color="black", relative=False
    )
    fig.legend()
    ...
    print(
        est.system_capture(source_bg)
    )  # this is the absolute light-induced capture for the background intensities
    ...
    print(
        est.in_hull(B).all()
    )  # all previously sampled values should be within the hull
    ...
    fig, axes = est.gamut_plot(
        B2, colors=sources_colors, c="tomato", alpha=0.5, axes=axes
    )
    plt.close()
    ...
    # important test
    assert np.allclose(
        Bnewhat1, est.system_relative_capture(X)
    ), "either system relative capture is inaccurate or predicted capture calculation is wrong"
    ...
    fig, axes = est.gamut_plot(
        Bnewhat1, colors=sources_colors, c=colors, marker="x", alpha=1, axes=axes
    )
    fig, axes = est.gamut_plot(
        Bnewhat2, colors=sources_colors, c=colors, marker="s", alpha=1, axes=axes
    )
    fig, axes = est.gamut_plot(
        Bnewhat3, colors=sources_colors, c=colors, marker="+", alpha=1, axes=axes
    )
    plt.close()
...
def test_uncertainty():
    ...
    sns.heatmap(est.Epsilon, ax=ax1, vmin=0, cmap="viridis")
    sns.heatmap(est.A, ax=ax2, vmin=0, cmap="viridis")
    plt.close()
...
def test_gamut_corrections():
    ...
    image_wls = np.load(
        os.path.join(os.path.dirname(__file__), "data", "wls_image.npy")
    )
    ...
    est.gamut_plot(
        B,
        colors=sources_colors,
        ncols=3,
        fig_kws={"figsize": (15, 5)},
        c="gray",
        alpha=0.5,
    )
    plt.close()
    ...
    _, scales, Bscaled_fit = est.fit_adaptive(
        ...
        scale_w=np.array(
            [0.001, 10]
        ),  # trade-off between intensity and chromatic value for max objective
    )
