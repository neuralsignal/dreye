...
To build the docs you must have all optional dependencies for dreye
...
class DocBuilder:
    ...
    def __init__(
        ...
        if single_doc and single_doc.endswith(".rst"):
            ...
            self.single_doc_html = f"reference/api/dreye.{single_doc}.html"

    def _process_single_doc(self, single_doc):
        """
        Make sure the provided value for --single is a path to an existing
        .rst/.ipynb file, or a dreye object that can be imported.

        For example, categorial.rst or dreye.FrameEngine. For the latter,
        return the corresponding file path
        (e.g. reference/api/dreye.FrameEngine.rst).
        """
        ...
        elif single_doc.startswith("dreye."):
            try:
                obj = dreye  # noqa: F821
                ...
                return single_doc[len("dreye.") :]
        else:
            raise ValueError(
                f"--single={single_doc} not understood. "
                "Value should be a valid path to a .rst or .ipynb file, "
                "or a valid pufbbird object "
                "(e.g. categorical.rst or dreye.DataFrame.head)"
            )
...
<html>"""
    ...
    def html(self):
        ...
        zip_fname = os.path.join(BUILD_PATH, "html", "dreye.zip")
    ...
    def latex(self, force=False):
        ...
        if sys.platform == "win32":
            ...
            if force:
                for i in range(3):
                    self._run_os("pdflatex", "-interaction=nonstopmode", "dreye.tex")
                raise SystemExit(
                    "You should check the file "
                    '"build/latex/dreye.pdf" for problems.'
                )
            else:
                self._run_os("make")
            return ret_code
...
def main():
    ...
    argparser = argparse.ArgumentParser(
        description="dreye documentation builder", epilog=f"Commands: {joined}",
    )
    ...
    globals()["dreye"] = importlib.import_module("dreye")
